...
@Component({
  selector: 'app-medicalselfrequest',
  templateUrl: './medicalselfrequest.component.html',
  styleUrls: ['./medicalselfrequest.component.css']
})
export class MedicalselfrequestComponent {
...
spouses:Spouse[]=[]
  ...
  id:string;
 selectedEmployee: string='c6b2f0a9-8af0-473b-820b-73e47628189f'; 
  employees:Employee[]=[]; 
...
.subscribe({  
  next: (posti) => { 
    // this.leaveRequest.empId = this.selectedEmployee; 
  this.employeepostion=posti
    console.log("postions  jkkgy",this.employeepostion)
    this.selectedPosition = this.getPositionName(this.employeepostion.position)
console.log("jkkgy",this.employeepostion.position)
this.selectedEmployeepostion=this.employeepostion.position

const selectedDivision=this.employeepostion.divisionId 
...
if(((this.medicalRequest.spouseId== null )&& 
(this.selfMedBalance>0))||((ss?ss.familyBalance>0:false) && 
(this.medicalRequest.spouseId != null )))
 {
    this.medicalRequest.empId = this.selectedEmployee;
    this.medicalRequest.createdBy=this.selectedEmployee;
    ...
    this.medicalRequestService.addMedicalRequest(this.medicalRequest).subscribe({ 
      next: (employee) => { 
        ...
        this.medicalRequestSaved=true 
        setTimeout(() => { 
          this.medicalRequestSaved = false; 
        }, 2000); 
...
editmedicalRequest(medicalRequest: MedicalRequests): void { 
   ...
   this.medicalRequest.spouseId=this.selectedSpouse
    this.medicalRequestService.updateMedicalRequest(this.medicalRequest, this.medicalRequest.medicalRequestId).subscribe({ 
      next: () => { 
        ...
        this.medicalRequestService.getMedicalRequestByEmp(this.selectedEmployee).subscribe({ 
      next: (medicalRequestd) => { 
        this.medicalRequests = medicalRequestd.filter(med=>med.createdBy==this.selectedEmployee); 
  ...
  deletemedicalRequest(medicalreq:MedicalRequests){
    const dialogRef = this.dialog.open(DeleteConfirmationComponent); 
 
    dialogRef.afterClosed().subscribe((result) => { 
      if (result === true) { 
        this.medicalRequestService.deleteMedicalRequest(medicalreq.medicalRequestId).subscribe({
          next: () => { 
